...
function loadRecords(filter = null) {
    ...
    const displayRecords = filter 
        ? sortedRecords.filter(record => 
            ...
            record.amount.toString().includes(filter)
          )
        : sortedRecords;
...
function updateSummary() {
    ...
    const surplusElement = document.querySelector('.surplus');
    surplusElement.textContent = `¥${totalSurplus.toFixed(2)}`;

    surplusElement.classList.remove('positive', 'zero', 'negative');
    if (totalSurplus > 0) {
        surplusElement.classList.add('positive');
    } else if (totalSurplus === 0) {
        surplusElement.classList.add('zero');
    } else {
        surplusElement.classList.add('negative');
    }    

    document.querySelector('.monthIncome').textContent = `¥${monthIncome.toFixed(2)}`;
    document.querySelector('.monthExpense').textContent = `¥${monthExpense.toFixed(2)}`;
    const monthSurplusElement = document.querySelector('.monthSurplus');
    monthSurplusElement.textContent = `¥${monthSurplus.toFixed(2)}`;

    monthSurplusElement.classList.remove('positive', 'zero', 'negative');
    if (monthSurplus > 0) {
        monthSurplusElement.classList.add('positive');
    } else if (monthSurplus === 0) {
        monthSurplusElement.classList.add('zero');
    } else {
        monthSurplusElement.classList.add('negative');
    }
    ...
    document.querySelectorAll('.showCard').forEach(card => {
        ...
        if (sources.hasOwnProperty(title)) {
            const sourceElement = card.querySelector('h3');
            sourceElement.textContent = `¥${sources[title].toFixed(2)}`;

            sourceElement.classList.remove('positive', 'zero', 'negative');
            
            if (sources[title] > 0) {
                sourceElement.classList.add('positive');
            } else if (sources[title] === 0) {
                sourceElement.classList.add('zero');
            } else {
                sourceElement.classList.add('negative');
            }
        }
    });
}
